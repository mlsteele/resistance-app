...
var RoleCard = React.createClass({
    ...
    render: function() {
        ...
        var oberonText = role.hasOberon? <Text><br /><Text className='spy'>Oberon</Text> is hidden from you.</Text> : ''
        var spiesBlock = theSpies.length > 0
                ? <Text>The {other} {spyNoun} {spyVerb} <Text className='spy'>{spiesText}</Text>. {oberonText}</Text>
                : <Text>You do not see any {other} spies.</Text>
        var extraInfo = <View></View>
        var description = <Text></Text>

        var name = <Text className='resistance'>resistance</Text>

        if (role.spy && !role.oberon) {
            name = <Text>a <Text className='spy'>spy</Text></Text>;
            extraInfo = spiesBlock;
        }
        if (role.percival) {
            name = <Text className='resistance'>Percival</Text>
            ...
            description = <Text>You see <Text className='resistance'>Merlin</Text> and <Text className='spy'>Morgana</Text> both as Merlin.</Text>
        }
        if (role.merlin) {
            name = <Text className='resistance'>Merlin</Text>;
            extraInfo = spiesBlock;
            description = <Text>If the spies discover your identity, resistance loses!</Text>
        }
        if (role.mordred) {
            name = <Text className='spy'>Mordred</Text>
            description = <Text>You are invisible to <Text className='resistance'>Merlin</Text>.</Text>
        }
        if (role.morgana) {
            name = <Text className='spy'>Morgana</Text>
            description = <Text>You appear as <Text className='resistance'>Merlin</Text> to <Text className='resistance'>Percival</Text>.</Text>
        }
        if (role.oberon) {
            name = <Text className='spy'>Oberon</Text>
