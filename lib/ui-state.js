...
UIState.prototype.save = function() {
    ...
    LocalStore.set('store.uistate', persist).done()
}

UIState.prototype.load = async function() {
    const persist = await LocalStore.get('store.uistate')
...
UIState.actions.hasLoaded = function() {
    this.loading = false
    this.emitChange()
}
